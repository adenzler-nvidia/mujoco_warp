...
def _geom_filter(
  ...
  contype1 = m.geom_contype[worldid % m.geom_contype.shape[0], geom1]
  contype2 = m.geom_contype[worldid % m.geom_contype.shape[0], geom2]
  conaffinity1 = m.geom_conaffinity[worldid % m.geom_conaffinity.shape[0], geom1]
  conaffinity2 = m.geom_conaffinity[worldid % m.geom_conaffinity.shape[0], geom2]
...
def broadphase_project_spheres_onto_sweep_direction_kernel(
  ...
  if r == 0.0:
    ...
    r = 1000000000.0
  sphere_radius = r + m.geom_margin[worldid % m.geom_margin.shape[0], i]
...
def reorder_bounding_spheres_kernel(
  ...
  margin = m.geom_margin[worldid % m.geom_margin.shape[0], mapped]
...
def get_contact_solver_params_kernel(
  ...
  margin = wp.max(m.geom_margin[worldid % m.geom_margin.shape[0], g1], m.geom_margin[worldid % m.geom_margin.shape[0], g2])
  gap = wp.max(m.geom_gap[worldid % m.geom_gap.shape[0], g1], m.geom_gap[worldid % m.geom_gap.shape[0], g2])
  solmix1 = m.geom_solmix[worldid % m.geom_solmix.shape[0], g1]
  solmix2 = m.geom_solmix[worldid % m.geom_solmix.shape[0], g2]
  mix = solmix1 / (solmix1 + solmix2)
  mix = wp.where((solmix1 < MJ_MINVAL) and (solmix2 < MJ_MINVAL), 0.5, mix)
  mix = wp.where((solmix1 < MJ_MINVAL) and (solmix2 >= MJ_MINVAL), 0.0, mix)
  mix = wp.where((solmix1 >= MJ_MINVAL) and (solmix2 < MJ_MINVAL), 1.0, mix)

  p1 = m.geom_priority[worldid % m.geom_priority.shape[0], g1]
  p2 = m.geom_priority[worldid % m.geom_priority.shape[0], g2]
  ...
  solref_g1 = m.geom_solref[worldid % m.geom_solref.shape[0], g1]
  solref_g2 = m.geom_solref[worldid % m.geom_solref.shape[0], g2]
  if solref_g1.x > 0.0 and solref_g2.x > 0.0:
    d.contact.solref[tid] = (
      mix * solref_g1 + (1.0 - mix) * solref_g2
    )
  else:
  ...
  friction_ = wp.max(m.geom_friction[worldid % m.geom_friction.shape[0], g1], m.geom_friction[worldid % m.geom_friction.shape[0], g2])
  ...
  d.contact.solimp[tid] = (
    mix * m.geom_solimp[worldid % m.geom_solimp.shape[0], g1] + (1.0 - mix) * m.geom_solimp[worldid % m.geom_solimp.shape[0], g2]
  )
...
def nxn_broadphase(m: Model, d: Data):
  ...
  def _nxn_broadphase(m: Model, d: Data):
    ...
    margin1 = m.geom_margin[worldid % m.geom_margin.shape[0], geom1]
    margin2 = m.geom_margin[worldid % m.geom_margin.shape[0], geom2]
