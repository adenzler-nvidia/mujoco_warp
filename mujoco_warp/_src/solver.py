...
def create_solver_context(m: types.Model, d: types.Data) -> SolverContext:
  ...
  nv_pad = m.nv_pad
  njmax = d.njmax
  njmax_pad = d.efc.state.shape[1]  # padded constraint dimension
...
def incremental_h_update(
  ...
  # In/Out:
  ctx_h_inout: wp.array3d(dtype=float),
):
...
def _compute_gradient(m: types.Model, d: types.Data, ctx: SolverContext):
  """Compute grad = Ma - qfrc_smooth - qfrc_constraint."""
...
def _compute_hessian_and_factor(m: types.Model, d: types.Data, ctx: SolverContext, skip=None):
  """Compute H = qM + J^T*D*J, Cholesky factor, and solve for Mgrad.
  """
  done = skip if skip is not None else ctx.done
  ...
  if m.is_sparse:
    ...
    wp.launch(
      ...
      inputs=[m.qM_fullm_i, m.qM_fullm_j, d.qM, done],
      outputs=[ctx.h],
    )
  else:
  ...
  if m.nv <= _BLOCK_CHOLESKY_DIM:
    ...
    wp.launch_tiled(
      ...
      dim=d.nworld,
      inputs=[done, ctx.grad.reshape(shape=(d.nworld, ctx.grad.shape[1], 1)), ctx.h, ctx.hfactor],
...
def _update_gradient(m: types.Model, d: types.Data, ctx: SolverContext):
  """Full gradient + Hessian computation (original path)."""
  _compute_gradient(m, d, ctx)

  if m.opt.solver == types.SolverType.CG:
    smooth.solve_m(m, d, ctx.Mgrad, ctx.grad)
  elif m.opt.solver == types.SolverType.NEWTON:
    _compute_hessian_and_factor(m, d, ctx)
  else:
...
def _update_gradient_incremental(m: types.Model, d: types.Data, ctx: SolverContext):
  ...
  Updates H in-place for changed constraints (early-returns for worlds with
  no changes), then always re-factorizes and solves using the regular cholesky
  kernel. No stored factor needed — H is the source of truth.
  """
  _compute_gradient(m, d, ctx)
  ...
  # Factorize H and solve (same kernels as main uses)
  if m.nv <= _BLOCK_CHOLESKY_DIM:
    wp.launch_tiled(
      update_gradient_cholesky(m.nv),
      dim=d.nworld,
