...
def passive(m: Model, d: Data):
  ...
  def _spring(m: Model, d: Data):
    ...
    stiffness = m.jnt_stiffness[worldid % m.jnt_stiffness.shape[0], jntid]
    ...
    if jnt_type == wp.static(JointType.FREE.value):
      dif = wp.vec3(
        d.qpos[worldid, qposid + 0] - m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 0],
        d.qpos[worldid, qposid + 1] - m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 1],
        d.qpos[worldid, qposid + 2] - m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 2],
      )
      ...
      ref = wp.quat(
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 3],
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 4],
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 5],
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 6],
      )
      ...
      ref = wp.quat(
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 0],
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 1],
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 2],
        m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid + 3],
      )
      dif = math.quat_sub(rot, ref)
      d.qfrc_spring[worldid, dofid + 0] = -stiffness * dif[0]
      ...
      fdif = d.qpos[worldid, qposid] - m.qpos_spring[worldid % m.qpos_spring.shape[0], qposid]
      d.qfrc_spring[worldid, dofid] = -stiffness * fdif

  @kernel
  def _damper_passive(m: Model, d: Data):
    worldid, dofid = wp.tid()
    damping = m.dof_damping[worldid % m.dof_damping.shape[0], dofid]
